...
class SisService
{
    ...
    {
        $phoneNumber = preg_replace(self::PHONE_NUMBERS_PREFIX_REGEX, '', $phoneNumber);

        $this->statement = $this->sisConnection->prepare('
                SELECT
                    slpc.*
                FROM
                    sis.student_list_phone_calls slpc
                WHERE 
                    phone LIKE :phoneNumber');
        $this->statement->bindValue('phoneNumber', '%' . $phoneNumber . '%');
